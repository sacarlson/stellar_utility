...
re = Utils.create_unlock_transaction(multi_sig_account_keypair,signerC_keypair,(Time.now.to_i + 10))
puts "re:  #{re}"
Utils.view_envelope(re["unlock_env_b64"])
exit -1
if 1 == 0
#def add_signer_and_weight_manual(target_keypair,add_address,weight_signer,weight_med_high)
b64 = Utils.add_signer_and_weight_manual(multi_sig_account_keypair,signerA_keypair,1,1)
result = Utils.send_tx(b64)
puts "result send_tx:  #{result}"
b64 = Utils.add_signer_and_weight_manual(multi_sig_account_keypair,signerC_keypair,1,1)
result = Utils.send_tx(b64)
puts "result send_tx:  #{result}"
re = Utils.create_unlock_transaction(multi_sig_account_keypair,signerC_keypair,(Time.now.to_i + 10))
puts "re:  #{re}"
