...
EM.run {
  ...
  class Server < Sinatra::Base
    ...
    post '/?' do 
      ...
      when "send_native"
        begin
          results =  mult_sig.Utils.send_native(Stellar::KeyPair.from_seed(request_payload["from_seed"]), request_payload["to_account"], request_payload["amount"], request_payload["memo_text"])
        rescue
      ...
      when "send_asset"
        begin
          results =  mult_sig.Utils.send_currency(Stellar::KeyPair.from_seed(request_payload["from_seed"]), request_payload["to_account"],request_payload["issuer"], request_payload["amount"],request_payload["assetcode"],request_payload["memo_text"])
        rescue
          results = {"action"=>"send_native","status"=>"error" ,"error"=>"bad input"}
        end
        results.to_json
